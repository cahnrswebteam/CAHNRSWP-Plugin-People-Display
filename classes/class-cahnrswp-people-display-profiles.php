...
//	 	$profiles_list = $this->get_profiles( $atts );	
	...
	*/	
	...
	//	$sc_classificaton = "faculty";
		...
		if ( $university_category ) {$university_category_code = '&filter[university_category_slug]=' . $university_category;}
...
//		$url = "https://people.wsu.edu/wp-json/wp/v2/people/?type=wsuwp_people_profile&filter[orderby]=title&filter[order]=ASC&filter[classification]=' . $sc_classificaton . '&filter[wsuwp_university_location]=mount-vernon&per_page=100";
		...
		$url = "https://people.wsu.edu/wp-json/wp/v2/people/?type=wsuwp_people_profile" . $organization_code ."&filter[orderby]=title&filter[order]=ASC&" . $classificaton_code . '"' . $location_code . '"'. $tag_code . '"' . $university_category_code . "&per_page=100";
